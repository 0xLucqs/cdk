...
/// returns `false`.
pub fn verify(px: u256, rx: u256, s: u256, m: ByteArray) -> Result<u256, felt252> {
    ...
    let P =
        match Secp256Trait::<Secp256k1Point>::secp256_ec_get_point_from_x_syscall(px, false)
            .unwrap_syscall() {
        Option::Some(P) => P,
        Option::None => { return Result::Err(2); }
    };
...
mod tests {
    ...
    fn test_0() {
        ...
        let m: u256 = 0x0;
        assert!(verify(px, rx, s, m.into()).is_ok());
    }
    ...
    fn test_5() {
        ...
        let m: u256 = 0x243f6a8885a308d313198a2e03707344a4093822299f31d0082efa98ec4e6c89;
        assert_eq!(verify(px, rx, s, m.into()).is_ok(), false);
    }
    ...
    fn test_20() {
         let px = u256 {
        low: 89062226256809426899172950455735654985,
        high: 279089910671712443122619726700590065620
    };
